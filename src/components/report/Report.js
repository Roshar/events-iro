...
const Report = ({
  ...
  const getActual = (idx) => {
    return parseInt(idx) == 2 ? "Прошедшие" : "Планируются";
  };
  ...
  const submitEvents = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/admin/report/${type}`,
        formData,
        {
          headers: {
        ...
        <div className="report__filter_group">
          ...
          <div className="report__tbl-header">
            {result !== false ? (
              <>
                <h2>
                  Статистика по {statText} за {year} год
                </h2>
                <span className="report__description">
              ...
              <tbody>
                ...
                <tr className="report__tbl_tr">
                  ...
                  <td className="report__tbl_td">
                    {getMonth(month, resMonth)}
                  </td>
                </tr>
                <tr className="report__tbl_tr">
                  <th className="report__tbl_td">Категория:</th>
                  <td className="report__tbl_td">
                    {getCat(categories, resCat)}
                  </td>
                </tr>
                <tr className="report__tbl_tr">
                ...
                <tr className="report__tbl_tr">
                  <th className="report__tbl_td">
                    {" "}
                    Структурное подразделение:
                  </th>
                  <td className="report__tbl_td">
                    {getCenter(centers, resCen)}
                  </td>
                </tr>
                <tr className="report__tbl_tr">
