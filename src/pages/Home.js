...
const Home = () => {
  ...
  const handleYear = (e) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/events/cat/${JSON.stringify(params)}`
      )
      .then((response) => {
  ...
  const handleOrg = (e) => {
    ...
    axios
      ...
      .then((response) => {
        setAPIDataSoon(response.data.soon);
        response.data.orgReult
          ? setOrgName(response.data.orgReult[0]["name"])
          : setOrgName("");
      });
  };
  ...
  const handleMonthLast = (e) => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_BASE_URL}/events/cat/last/${JSON.stringify(
          paramsLast
        )}`
      )
      .then((response) => {
  ...
  const handleOrgLast = (e) => {
    ...
    axios
      ...
      .then((response) => {
        setAPIDataLast(response.data.last);
        response.data.orgReult
          ? setOrgNameLast(response.data.orgReult[0]["name"])
          : setOrgNameLast("");
      });
  };
  ...
  const moreSoon = () => {
    ...
    if (offset == 0) {
      ...
      axios
        .get(
          `${process.env.REACT_APP_BASE_URL}/events/cat/${JSON.stringify(
            params
          )}`
        )
        .then((response) => {
          if (response.data.soon.length > 0) {
            let result = Array.from(
              new Set([...response.data.soon, ...APIDataSoon])
            );
            console.log(result);
      ...
      axios
        ...
        .then((response) => {
          if (response.data.last.length > 0) {
            let result = Array.from(
              new Set([...response.data.last, ...APIDataLast])
            );
      ...
      <main className="main">
        ...
        <div className="container">
          <div className="main__header">
            <h2 className="home-content__title" id="soonBlock">
              Скоро
            </h2>
              ...
              <div className="filters__wrapper">
                <div className="filters__header">
                  <select
                    ...
                    onChange={handleYear}
                  >
                    <option value="2024" className="filters__option">
                      2024
                    </option>
                    <option value="2023" className="filters__option">
                      2023
                    </option>
                    <option value="2022" className="filters__option">
                      2022
                    </option>
                  </select>
                  ...
                  >
                    <option value="" className="filters__option">
                      {" "}
                      Месяц
                    </option>
                    {getMonthList.map((el) => {
        ...
        <div className="events">
          ...
          <div className="moreList" href="#soonBlock" onClick={moreSoon}>
            {" "}
            Загрузить еще
          </div>
        </div>
        ...
        <div className="events">
          ...
          <div className="moreList" onClick={moreLast}>
            {" "}
            Загрузить еще
          </div>
        </div>
