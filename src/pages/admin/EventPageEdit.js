...
const EventPageEdit = () => {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_BASE_URL}/admin/event/edit/${id}`).then((response) => {
            ...
            getAllSpeakers(response.data.speakersList, response.data.speakersForEvent)
                    ...
                    <div className="admin_event">
                        <form className="admin_event__form">
                            <div className="admin_event__form-control">
                                ...
                                <input className="admin_event__input" type="text" id="title" name="title" onChange={handleChange} value={title} />
                            </div>
                            <div className="admin_event__form-control">
                                <label className="admin_event__label" htmlFor="description" >Краткое описание:</label>
                                <textarea className="admin_event__area" type="text" id="description" name="description" rows="9" onChange={handleChange} value={description} />
                            </div>

                            <div className="admin_event__form-control">
                                <label className="admin_event__label" htmlFor="description" >Категория мероприятия: </label>
                                <select className="admin_event__select" name="category_id" id="category_id" onChange={handleChange}>
                                    {catList.map((el) => {
                                        return (
                                            <option className="admin_event__option" key={el.id} value={el.id} selected={el.id == categoryId}> {el.cat_name} </option>
                                        )
                                    })}
                            ...
                            <div className="admin_event__form-control">
                                ...
                                <select className="admin_event__select" name="category_id" id="category_id" onChange={handleChange}>
                                    {organizationsList.map((el) => {
                                        return (
                                            <option className="admin_event__option" key={el.id} value={el.id} selected={el.id == orgId}> {el.name} </option>
                                        )
                                    })}
                                </select>
                            </div>
                            ...
                            <div className="admin_event__form-control">
                                ...
                                <input className="admin_event__input" type="date" id="date_event" name="date_event" value={dateEvent} min="2023-01-01" max="2024-12-31" onChange={handleChange} />
                            </div>

                            <div className="admin_event__form-control">
                                <label className="admin_event__label" htmlFor="time_event" >Время проведения:</label>
                                <input className="admin_event__input" type="time" id="time_event" name="time_event" onChange={handleChange} value={time} />
                            </div>

                            <div className="admin_event__form-control">
                                <label className="admin_event__label" htmlFor="date_event" >Место проведения:</label>
                                <input className="admin_event__input" type="text" id="location" name="location" onChange={handleChange} value={location} />
                            </div>

                            <div className="admin_event__form-control">
                                <label className="admin_event__label" htmlFor="target_audience" >Целевая аудитория:</label>
                                <input className="admin_event__input" type="text" id="target_audience" name="target_audience" onChange={handleChange} value={target} />
                            </div>

                            <div className="admin_event__form-control">
                                <label className="admin_event__label" htmlFor="participants_number" >Рассчитано на количество человек:</label>
                                <input className="admin_event__input" type="text" id="participants_number" name="participants_number" onChange={handleChange} value={parNumber} />
                            </div>
                                        ...
                                        {filterSpeakers.map((el) => {
                                            return (
                                                ...
                                                <li className="admin_event__item" key={el.id}>
                                                    ...
                                                    <button className="admin_event__btn btn btn--admin" type="button" id="notAssigned" onClick={changeSpeakers}>Добавить</button>
                                                </li>
                                        ...
                                        {speakersCurrent.map((el) => {
                                            return (
                                                <li className="admin_event__item" key={el.id}>
                                                    ...
                                                    <button className="admin_event__btn btn btn--admin" type="button" id="assigned" onClick={changeSpeakers} >Добавить</button>
                                                </li>
                                            )
                                        })}
                                    </ul>
                            ...
                            <div className="admin_event__form-control">
                                ...
                                <select className="admin_event__select" name="event_status" id="event_status" onChange={handleChange}>
                                    <option className="admin_event__option" value='1' key={1} selected={1 == statusPub}> Опубликованно </option>
                                    <option className="admin_event__option" value='2' key={2} selected={2 == statusPub}> Неопубликовано </option>
                                </select>
                            </div>
